...
import { generateQuotationDraft } from '../services/aiQuotationService.js';
...
export const generateAIDraft = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    const { draft, rawResponse } = await generateQuotationDraft({
      prompt: prompt.trim(),
      clientId,
    ...
    res.json({
      ...
      draft: {
        ...
        supportDetails: draft.supportDetails,
    ...
    res.status(500).json({ 
      error: 'Failed to generate quotation draft',
      details: process.env.NODE_ENV === 'development' ? error.message : undefined,
    });
  }
};
